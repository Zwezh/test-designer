...
import { authReducers, AuthState, getCurrentTeacherAction, getCurrentTeacherFailueAction, getCurrentTeacherSuccessAction, getTeacherCollectionFailureAction, getTeacherCollectionSuccessAction, loginAction, loginFailueAction, loginSuccessAction, logoutAction, registerAction, registerFailueAction, registerSuccessAction, updateCurrentTeacherAction, updateCurrentTeacherFailueAction, updateCurrentTeacherSuccessAction } from '@appStore';
...
describe('Auth Reducer', () => {
  ...
  it('Should return the updated state with loading "false" and "new collection in state" by "get success current teacher" action', () => {
    const updatedCollecion = [...initialState.teacherCollection, { ...expectedTeacher, name: 'Joe' }]
    const state = authReducers(initialState, getTeacherCollectionSuccessAction({ teacherCollection: updatedCollecion }));
  ...
  it('Should return the updated state with loading "false", loggedIn "true" and currentTeacher by "login success" action', () => {
    const state = authReducers(initialState, loginSuccessAction({ currentTeacher: expectedTeacher }));
    expect(state.isLoading).toBe(false);
  ...
  it('Should return the updated state with loading "false", loggedIn "true" and new currentTeacher by "register success" action', () => {
    const state = authReducers(initialState, registerSuccessAction({ teacher: expectedTeacher }));
    expect(state.isLoading).toBe(false);
    expect(state.currentTeacher).toEqual(expectedTeacher);
    expect(state.isLoggedIn).toBe(true);
  });
  ...
  it('Should return the updated state with loading "false", loggedIn "true" and currentTeacher by "get current teacher success" action', () => {
    const state = authReducers(initialState, getCurrentTeacherSuccessAction({ currentTeacher: expectedTeacher }));
    expect(state.isLoading).toBe(false);
    expect(state.currentTeacher).toEqual(expectedTeacher);
    expect(state.isLoggedIn).toBe(true);
  });
  ...
  it('Should return the updated state with loading "false" and currentTeacher by "update teacher success" action', () => {
    const state = authReducers(initialState, updateCurrentTeacherSuccessAction({ currentTeacher: expectedTeacher }));
    expect(state.isLoading).toBe(false);
    expect(state.currentTeacher).toEqual(expectedTeacher);
  });
